...
$mod=@return_edefualt(str_callback_w($_GET['mod']),'usedcarmall');
$file=@return_edefualt(str_callback_w($_GET['file']),'usedcarmall');
...
switch($action)
{
	...
	case 'config':
		if(isset($dosubmit))
		{
			...
			operation_tips('操作成功！','?mod=usedcarmall&file=usedcarmall&action=config');
		}
	...
	//case 'manage':
		...
		//在此写 phpwechat.php?mod=usedcarmall&file=usedcarmall&action=manage 时的逻辑
			
		//break;

	//case 'add':
			
		//在此写 phpwechat.php?mod=usedcarmall&file=usedcarmall&action=add 时的逻辑
	...
	//case '...':
		...
		//在此写 phpwechat.php?mod=usedcarmall&file=usedcarmall&action=... 时的逻辑
